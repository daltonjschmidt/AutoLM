...
def combine_users(users_df, league_df, rosters_df,players_df):
    ...
    player_cols = league_cols + ['player_id', 'full_name', 'first_name', 'last_name', 'team', 'position', 'age', 'height', 'weight', 'status','roster_year']
    ...
    roster_data = roster_data.merge(players_df, on='player_id', suffixes=['',"_p"],how='left')
    roster_data_filtered = pd.DataFrame(roster_data, columns=player_cols)
    ...
    return league_data, roster_data_filtered
...
def combine_player_performances(players_df, player_performance_df, combined_league_df):
    ...
    perf_cols = ['year_week_matchup', 'year','week_number', 'roster_id','roster_year', 'matchup_id', 'player_id', 'player_points', 'starter','display_name', 'team_name']
    player_cols = perf_cols + ['full_name', 'first_name', 'last_name', 'team', 'position', 'age', 'roster_year']
...
#pprint(get_drafts(920151115868565504))


#league, rosters = get_rosters(920151115868565504,2023)
#filtered = rosters[rosters['player_id'] == '8183']
#pprint(filtered)

#users = users_api()
#players = fill_players()
#league, rosters = get_rosters()
#player_performances, weekly_summary = get_matchups()
#transactions = get_transactions()

#combined_league, combined_roster = combine_users(users, league, rosters,players)
#combined_weekly_summaries = combine_weekly_matchup_summaries(combined_league, weekly_summary)
#combined_pp = combine_player_performances(players, player_performances, combined_league)
#combined_transactions = combine_transactions(transactions, players, combined_league)


#path_league = f'C:\\Users\\schmd\\Documents\\AutoLM\\2023\\league_2023.csv'
#path_rosters = f'C:\\Users\\schmd\\Documents\\AutoLM\\2023\\rosters_2023.csv'
#path_performances = f'C:\\Users\\schmd\\Documents\\AutoLM\\2023\\player_performances_2023.csv'
#path_weekly_summaries = f'C:\\Users\\schmd\\Documents\\AutoLM\\2023\\weekly_summaries_2023.csv'
#path_transactions = f'C:\\Users\\schmd\\Documents\\AutoLM\\2023\\transactions_2023.csv'


#combined_league.to_csv(path_league, index=False)
...
#Evaluate data for all time

#all_users = pd.DataFrame()
#path = f'C:\\Users\\schmd\\Documents\\AutoLM\\League Data\\users_all_time.csv'
#for league_id, league_year in leagues.items():
#    league_number = league_id
#    year = league_year
#    all_users = pd.concat([all_users, users_api()], ignore_index=True)
#all_users.to_csv(path, index=False)
...
#path = f'C:\\Users\\schmd\\Documents\\AutoLM\\League Data\\leagues_all_time.csv'
#path1 = f'C:\\Users\\schmd\\Documents\\AutoLM\\League Data\\rosters_all_time.csv'
#for league_id, league_year in leagues.items():
#    league_number = league_id
#    year = league_year
#    league, roster = get_rosters()
#    all_leagues = pd.concat([all_leagues, league], ignore_index=True)
#    all_rosters = pd.concat([all_rosters, roster], ignore_index=True)
#all_leagues.to_csv(path, index=False)
#all_rosters.to_csv(path1, index=False)

#performance, week = get_matchups()
#player_performances = pd.DataFrame()
#weekly_summaries = pd.DataFrame()
#path = f'C:\\Users\\schmd\\Documents\\AutoLM\\League Data\\player_performance_all_time.csv'
#path1 = f'C:\\Users\\schmd\\Documents\\AutoLM\\League Data\\Weekly_summary_all_time.csv'
#for league_id, league_year in leagues.items():
#    league_number = league_id
#    year = league_year
#    performance, week = get_matchups()
#    player_performances = pd.concat([player_performances, performance], ignore_index=True)
#    weekly_summaries = pd.concat([weekly_summaries, week], ignore_index=True)
#player_performances.to_csv(path, index=False)
#weekly_summaries.to_csv(path1, index=False)


#all_transactions = pd.DataFrame()
#path = f'C:\\Users\\schmd\\Documents\\AutoLM\\League Data\\transactions.csv'
#for league_id, league_year in leagues.items():
#    league_number = league_id
#    year = league_year
#    all_transactions = pd.concat([all_transactions, get_transactions()], ignore_index=True)
